...
@interface LSFSDKHelperSceneManagerCallbackV2()
...
-(void)postUpdateSceneWithID: (NSString *)sceneID scene: (LSFSceneWithSceneElements *)scene;
...
@implementation LSFSDKHelperSceneManagerCallbackV2
...
-(void)getAllSceneIDsReplyWithCode: (LSFResponseCode)rc andSceneIDs: (NSArray *)sceneIDs
{
    if (rc != LSF_OK)
    {
        [self.manager.sceneElementCollectionManager sendErrorEvent: @"getAllSceneIDsReplyCB" statusCode: rc];
    }
...
-(void)getSceneNameReplyWithCode: (LSFResponseCode)rc sceneID: (NSString *)sceneID language: (NSString *)language andName: (NSString *)sceneName
{
    if (rc != LSF_OK)
    {
        [self.manager.sceneElementCollectionManager sendErrorEvent: @"getSceneNameReplyCB" statusCode: rc itemID: sceneID];
    }
...
-(void)setSceneNameReplyWithCode: (LSFResponseCode)rc sceneID: (NSString *)sceneID andLanguage: (NSString *)language
{
    if (rc != LSF_OK)
    {
        [self.manager.sceneElementCollectionManager sendErrorEvent: @"setSceneNameReplyCB" statusCode: rc itemID: sceneID];
    }
...
-(void)scenesNameChanged: (NSArray *)sceneIDs
{
    dispatch_async(self.manager.dispatchQueue, ^{
        ...
        for (NSString *sceneID in sceneIDs)
        {
            if ([self.manager.sceneElementCollectionManager hasID: sceneID])
            {
                [[LSFSDKAllJoynManager getSceneManager] getSceneNameWithID: sceneID andLanguage: self.manager.defaultLanguage];
            }
            else
...
-(void)updateSceneWithSceneElementsReplyWithCode: (LSFResponseCode)rc andSceneID: (NSString *)sceneID
{
    if (rc != LSF_OK)
    {
        [self.manager.sceneElementCollectionManager sendErrorEvent: @"updateSceneWithSceneElementsReplyCB" statusCode: rc itemID: sceneID];
    }
}
...
-(void)deleteSceneReplyWithCode: (LSFResponseCode)rc andSceneID: (NSString *)sceneID
{
    if (rc != LSF_OK)
    {
        [self.manager.sceneElementCollectionManager sendErrorEvent: @"deleteSceneReplyCB" statusCode: rc itemID: sceneID];
    }
}
...
-(void)getSceneWithSceneElementsReplyWithCode: (LSFResponseCode)rc sceneID: (NSString *)sceneID andSceneWithSceneElements: (LSFSceneWithSceneElements *)sceneWithSceneElements
{
    if (rc != LSF_OK)
    {
        [self.manager.sceneElementCollectionManager sendErrorEvent: @"getSceneWithSceneElementsReplyCB" statusCode: rc itemID: sceneID];
    }

    [self postUpdateSceneWithID: sceneID scene: sceneWithSceneElements];
}
...
{
    dispatch_async(self.manager.dispatchQueue, ^{
        BOOL hasID = [self.manager.sceneCollectionManager hasID: sceneID];
        if (!hasID)
        {
...
-(void)postUpdateSceneID: (NSString *)sceneID
{
    dispatch_async(self.manager.dispatchQueue, ^{
        ...
        if (!hasID)
        {
            [self.manager.sceneCollectionManager addSceneWithID: sceneID];
        }
    });
...
-(void)postDeleteScenes: (NSArray *)sceneIDs
{
    dispatch_async(self.manager.dispatchQueue, ^{
        for (NSString *sceneID in sceneIDs)
        {
            [self.manager.sceneCollectionManager removeSceneWithID: sceneID];
        }
    });
}
...
{
    dispatch_async(self.manager.dispatchQueue, ^{
        [self.manager.sceneCollectionManager sendChangedEvent: sceneID];
    });
}
...
-(void)postSendSceneInitialized: (NSString *)sceneID
{
    dispatch_async(self.manager.dispatchQueue, ^{
        ...
        [self.manager.sceneCollectionManager sendInitializedEvent: sceneID withTrackingID: trackingID];
    });
}
