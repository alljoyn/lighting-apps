...
#include "org_allseen_lsf_AboutData.h"
...
JNIEXPORT
jstring JNICALL Java_org_allseen_lsf_AboutData_getString(JNIEnv *env, jobject thiz, jlong jNativeAboutDataHandle, jstring jKey, jstring jLanguage)
{
    JString xKey(jKey);
...
JNIEXPORT
void JNICALL Java_org_allseen_lsf_AboutData_put(JNIEnv *env, jobject thiz, jlong jNativeAboutDataHandle, jstring jKey, jobject jVariant, jstring jLanguage)
{
    JString xKey(jKey);
    if (env->ExceptionCheck()) {
        QCC_LogError(ER_FAIL, ("JString failed"));
