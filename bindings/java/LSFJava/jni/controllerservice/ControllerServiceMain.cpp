...
extern void ControllerServiceSleep(uint32_t msec);
...
lsf::controllerservice::LSFAboutDataStore* controllerServiceDataStore = NULL;
lsf::controllerservice::ControllerServiceManager* controllerServiceManager = NULL;
    ...
    for (int i = 1; i < argc; ++i) {
        if (0 == strcmp("-h", argv[i]) || 0 == strcmp("-?", argv[i])) {
            ...
            if (i == argc) {
                ...
                pulseEffectFilePath = absDirPath + pulseEffectFilePath;
...
void RunService(bool listenToInterrupts)
{
    ...
    controllerServiceDataStore = new lsf::controllerservice::LSFAboutDataStore(factoryConfigFilePath.c_str(), configFilePath.c_str(), res);

    if (controllerServiceDataStore == NULL) {
        QCC_LogError(ER_OUT_OF_MEMORY, ("%s: Failed to allocate memory for LSFAboutDataStore", __func__));
        exit(-1);
    }

    lsf::controllerservice::ControllerServiceManager* controllerSvcManagerPtr =
        new lsf::controllerservice::ControllerServiceManager(
            factoryConfigFilePath,
            configFilePath,
            lampGroupFilePath,
            presetFilePath,
            transitionEffectFilePath,
            pulseEffectFilePath,
            sceneElementFilePath,
            sceneFilePath,
            sceneWithSceneElementFilePath,
            masterSceneFilePath);
    ...
    controllerServiceDataStore->SetControllerServicePtr(controllerSvcManagerPtr->GetControllerServicePtr());
    ...
    if (status == ER_OK) {
        while (g_running && controllerSvcManagerPtr->IsRunning()) {
            ControllerServiceSleep(OEM_CS_TIMEOUT_MS_CONNECTED_TO_ROUTING_NODE);
        }
    }
    ...
    if (controllerSvcManagerPtr) {
        ...
        delete controllerServiceDataStore;
        controllerServiceDataStore = NULL;
        delete controllerSvcManagerPtr;
        controllerSvcManagerPtr = NULL;
        controllerServiceManager = NULL;
...
int ControllerServiceMain(int argc, char** argv)
{
    QCC_SetDebugLevel("AJN-LSF-JNI", 7);
    QCC_DbgTrace(("%s", __func__));
