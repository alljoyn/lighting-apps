...
#ifdef LSF_JNI_XTRANSITIONEFFECTV10_H_INCLUDE_TEMPLATE_METHODS
...
template <typename T>
void XTransitionEffectV10::SetTransitionPeriod(JNIEnv *env, jobject thiz, jlong jTransitionPeriod)
{
...
template <typename T>
jlong XTransitionEffectV10::GetTransitionPeriod(JNIEnv *env, jobject thiz)
{
    T *xDelegate = GetHandle<T*>(thiz);
    if (env->ExceptionCheck() || !xDelegate) {
        QCC_LogError(ER_FAIL, ("GetHandle() failed"));
...
#endif /* LSF_JNI_XTRANSITIONEFFECTV10_H_INCLUDE_TEMPLATE_METHODS */
